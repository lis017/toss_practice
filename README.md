...
서버 뜨면 아래 명령어 복붙해서 바로 확인 가능합니다.

```bash
# 정상 결제 (캐시백 1,000원 적립)
curl -X POST http://localhost:8080/api/v1/payments \
  -H "Content-Type: application/json" \
  -d "{\"idempotencyKey\":\"test-001\",\"fromAccountId\":1,\"toAccountId\":2,\"amount\":10000}"

# 잔액 부족
curl -X POST http://localhost:8080/api/v1/payments \
  -H "Content-Type: application/json" \
  -d "{\"idempotencyKey\":\"test-002\",\"fromAccountId\":1,\"toAccountId\":2,\"amount\":999999999}"

# 멱등성 확인 (test-001 동일 키 재전송 → 첫 번째와 동일한 응답 반환, 계좌 이체 없음)
curl -X POST http://localhost:8080/api/v1/payments \
  -H "Content-Type: application/json" \
  -d "{\"idempotencyKey\":\"test-001\",\"fromAccountId\":1,\"toAccountId\":2,\"amount\":10000}"

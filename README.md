...
6. [테스트 실행](#테스트-실행)
7. [API](#api)
8. [장애 시나리오 테스트](#장애-시나리오-테스트)
9. [API 실행 테스트 .http 파일](#api-실행-테스트-http-파일)
10. [헬스체크](#헬스체크)
...
| F6 | 결제 상태 조회 API | `PaymentQueryController`, `PaymentQueryService` |
| F7 | 결제 완료 웹훅 발송 | `WebhookService`, `MockWebhookClient` |
| F8 | POST_PROCESS_FAILED 수동 복구 | `RecoveryService`, `RecoveryController` |
...
PaymentFacade.processPayment()
  └── WebhookService.sendPaymentCompleted()
        ├── MerchantSettlementPolicy 에서 가맹점 webhookUrl 조회
...
## POST_PROCESS_FAILED 수동 복구

외부 은행 승인 이후 내부 후처리(정산 레코드 생성, 상태 업데이트)가 실패한 경우 운영팀이 직접 복구합니다.

```
POST /api/v1/admin/payments/{transactionId}/recover
```

**복구 로직 (RecoveryService)**:
1. `POST_PROCESS_FAILED` 상태인지 검증 (아니면 `R001` 에러 반환)
2. `settlement_records` 누락이면 `SettlementService`로 재생성
3. `PaymentTransaction` 상태를 `PENDING_SETTLEMENT`로 업데이트

**안전성**: `@Transactional` — 복구 중 실패 시 전체 롤백 → 상태 변경 없음

### MockWebhookClient

실제 HTTP 호출 없이 로그만 남깁니다. 운영 전환 시 `WebhookClient` 인터페이스 구현체를 교체합니다.
...
```
GET http://localhost:8080/api/v1/payments/{transactionId}
```
...
- `settlementStatus`, `netAmount`는 결제 실패 케이스에서 `null`일 수 있습니다.
